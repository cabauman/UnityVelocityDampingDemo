// Sourced from: https://bitbucket.org/Unity-Technologies/ui/src/0bd08e22bc17bdf80bf7b997a4b43877ae4ee9ac/UnityEngine.UI/UI/Core/ScrollRect.cs?at=5.2
    ...
    protected virtual void LateUpdate()
    {
        ...
        if(!m_Dragging && (offset != Vector2.zero || m_Velocity != Vector2.zero))
        {
            ...
            for(int axis = 0; axis < 2; axis++)
            {
                ...
                {
                    float prevVelocity = m_Velocity[axis];
                    m_Velocity[axis] *= Mathf.Pow(m_DecelerationRate, deltaTime);
                    float avgVelocity = m_Velocity[axis] + (prevVelocity - m_Velocity[axis]) / 2;

                    if(Mathf.Abs(m_Velocity[axis]) < 1)
                        m_Velocity[axis] = 0;

                    position[axis] += avgVelocity * deltaTime;
                }
